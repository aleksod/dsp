...
```  
Find:  
```P(identical twins | twin boys)```  

>> Twins can be either monozygotic ("identical"), meaning that they develop from one zygote, which splits and forms two embryos, or dizygotic ("fraternal"), meaning that they develop from two different eggs. In fraternal twins, each twin is fertilized by its own sperm cell. (Twin - Wikipedia
https://en.wikipedia.org/wiki/Twin)  

>> Therefore,  
```
P(twin boys | fraternal twins) = 1/2 * 1/2 = 1/4
P(twin boys | identical twins) = 1/2```
>> Using Bayes' Theorem,  
```P(identical twins | twin boys) = (P(twin boys | identical twins) * P(identical twins)) / P(twin boys) ```  
`P(twin boys)` is the sum of all possible probabilities for twin boys. It consists of `P(twin boys | identical twins) * P(identical twins)` and of `P(twin boys | fraternal twins)`. Therefore,  
```
P(identical twins | twin boys) =
(P(twin boys | identical twins) * P(identical twins)) /
(P(twin boys | identical twins) * P(identical twins) + of P(twin boys | fraternal twins)) =
